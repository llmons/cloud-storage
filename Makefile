...
graph:
	...
	@cp -f cmd/cloud_storage/main.go .
	go-callvis -skipbrowser -format=svg -nostd -file=cloud_storage cloud-storage
	@rm -f main.go cloud_storage.gv
...
.PHONY: build
# Build cloud_storage for linux amd64 binary
build:
	@echo "building 'cloud_storage', linux binary file will output to 'cmd/cloud_storage'"
	@cd cmd/cloud_storage && CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build
